...
import { ArrowLeft, Save, Building2, Mail, Phone, ChevronsUpDown, Check, Search } from "lucide-react";
import { 
  Card, 
  CardContent, 
  CardDescription, 
...
export default function CreateOrganizationPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setErrors({ submit: response.error || 'Failed to create organization' });
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      {/* Page Header */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-green-600 via-teal-600 to-cyan-600 p-8 text-white">
        <div className="absolute inset-0">
          <div className="absolute bottom-0 left-0 w-32 h-16 bg-blue-500/20 rounded-tr-full"></div>
        ...
        <div className="relative z-10 flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-4xl font-bold tracking-tight mb-2">Create New Organization</h1>
            <p className="text-green-100 text-lg">
              Add a new organization to the TXS Invoice Management system
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4 sm:justify-end">
          ...
          <Button type="submit" disabled={isSubmitting} className="bg-gradient-to-r from-green-500 to-teal-600 hover:from-green-600 hover:to-teal-700 shadow-lg">
            {isSubmitting ? (
              <>Creating Organization...</>
            ) : (
              <>
                <Save className="mr-2 h-4 w-4 text-white" />
                <span className="text-white">Create Organization</span>
              </>
            )}
          </Button>
        </div>
